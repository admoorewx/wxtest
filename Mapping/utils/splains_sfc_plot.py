...
from metpy.plots import add_metpy_logo, current_weather, sky_cover, StationPlot,  colortables, wx_code_to_numeric
...
def main():
    ...
    ax.add_feature(cfeature.LAKES.with_scale('50m'), facecolor='cornflowerblue', edgecolor='k', linewidth=0.5, alpha=0.35, zorder=5)
    ...
    for i in range(0,len(data)):
        ...
        stationplot.plot_parameter('NE', [pres], c=prescolor, formatter=lambda v: format(10 * v, '.0f')[-3:], zorder=10.0)
        stationplot.plot_symbol('C', coverage, sky_cover, color=stationcolor,zorder=10.0)
        stationplot.plot_symbol('W', pres_wx, current_weather, fontsize=12, color=wxcolor, zorder=10.0)
    ...
    plt.savefig(saveDir+"splains_sfc_plot_"+radar_date+".png", bbox_inches='tight', pad_inches=0)
