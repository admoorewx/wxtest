...
from .wx_utilities import C2F, knot2mph, windChill, RH, Fosberg
...
def plot(site,previous):
    ...
    if metar is not None:
        ...
        # RH
        relh = [RH(Tc[i],Tdc[i]) for i in range(0,len(Tc))]
        # Winds
        wspd = metar.wspds
        wdir = metar.wdirs
        gust = metar.gusts
        ...
        # Fosberg Index
        fosberg = [Fosberg(temps[i],wspd[i],relh[i]) for i in range(0,len(temps))]
        # Precipitation
        precip = metar.precip
        ...
        elvwindline = [15.0 for x in range(0, len(temps))]
        critwindline = [20.0 for x in range(0, len(temps))]
        wind_buff = 5.0 #mph
        ...
        fig.append_trace(go.Scatter(x=times, y=elvwindline, line=dict(color='darkorange', dash='dash'), hoverinfo='skip', showlegend=False),row=2, col=1)
        fig.append_trace(go.Scatter(x=times, y=critwindline, line=dict(color='brown', dash='dash'), hoverinfo='skip', showlegend=False), row=2, col=1)
        ...
        relh_buff = 5.0 #(%)
        ymax = np.nanmax(relh) + relh_buff
        ymin = np.nanmin(relh) - relh_buff
        criticalline = [15.0 for x in range(0, len(temps))]
        elevatedline = [20.0 for x in range(0, len(temps))]
        fig.append_trace(go.Scatter(x=times, y=baseline, line=dict(color='black'), hoverinfo='skip', showlegend=False),row=3, col=1)
        fig.append_trace(go.Scatter(x=times,y=relh,name="Relative Humidity (%)",line=dict(color='forestgreen'),fill='tozeroy',showlegend=showleg),row=3,col=1)
        fig.append_trace(go.Scatter(x=times, y=elevatedline, line=dict(color='darkorange', dash='dash'), hoverinfo='skip', showlegend=False),row=3, col=1)
        fig.append_trace(go.Scatter(x=times, y=criticalline, line=dict(color='brown', dash='dash'), hoverinfo='skip', showlegend=False), row=3, col=1)

        fig.update_yaxes(range=[ymin,ymax],title="Rel. Humidity (%)",row=3,col=1)

        ### Plot the Fosberg Index ###
        fig.append_trace(go.Scatter(x=times,y=fosberg,name="Fosberg Index",
                                 line=dict(color='coral'),fill='tozeroy',showlegend=showleg),row=4,col=1)

        fig.update_yaxes(range=[0,100],title="Fosberg Index",row=4,col=1)
        ...
        fig.append_trace(go.Scatter(x=times,y=plot_precip,name="Liquid Precip (in)",
