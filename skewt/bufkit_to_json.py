...
model = "rap"
save_path = "/home/andrew/Python/forecast_soundings/json"
sounding_locations_file = "/home/andrew/wxtest/wxtest/static/data/soundings/forecast_locations.json"
...
def retrieve_data(station,stid,model,model_run_hour,save_path):
	...
	for i,line in enumerate(soup_lines):
		# The RAP forecast has either 21 or 51 hours worth of forecast data, depening on which
		# run it is (03, 09, 15, 21 UTC runs have 51 hours). However, collecting all 51 hours
		# takes a LONG time (nearly 45 mintues). We want this process to take ~20 min total. 
		# Implementing a check here to make sure we don't collect more than 21 hours of data.
		#if forecast_hour <= 21: # As of 05/26/23 taking this out for testing
		if "STID" in line:
			start = i 
...
for station, value in data.items():
	...
	try:
		...
		try:
			retrieve_data(stid,stid,model,hour,save_path)
			count = count + 1 
		except: 
