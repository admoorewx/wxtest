...
hgt_850 = hgt[idx_850]
tmp_850 = tmp[idx_850].to('degC')
rh_850 = relh[idx_850]
dewp_850 = mpcalc.dewpoint_from_relative_humidity(tmp_850,rh_850)

wspd_850 = gaussian_filter(mpcalc.wind_speed(u_850, v_850), 3)
hgt_850 = gaussian_filter(hgt_850, 3)
tmp_850 = gaussian_filter(tmp_850, 3)
dewp_850 = gaussian_filter(dewp_850, 3)
...
tmp850_f = ax.contourf(lon, lat, tmp_850, range(-20, 40, 4), alpha=0.5, transform=ccrs.PlateCarree(),
            cmap='coolwarm', zorder=3)
tmp850 = ax.contour(lon, lat, tmp_850, range(-20, 40, 4), alpha=0.7, linewidths=1, linestyles='dashed', transform=ccrs.PlateCarree(),
            colors='k', zorder=3)
ax.clabel(tmp850, tmp850.levels, inline=True, fontsize=8)

# Plot 850-hPa heights
cs850 = ax.contour(lon, lat, hgt_850, range(1300, 2000, 30), alpha=1.0, linewidths=2, linestyles='solid', transform=ccrs.PlateCarree(),
            colors='k', zorder=3)
ax.clabel(cs850, cs850.levels, inline=True, fontsize=8)

# Plot 850-hPa dewpoint
dewp850_f = ax.contourf(lon, lat, dewp_850, range(12, 24, 4), alpha=0.4, transform=ccrs.PlateCarree(),
            cmap='Greens', zorder=3)
dewp850 = ax.contour(lon, lat, dewp_850, range(4, 24, 2), alpha=1.0, linewidths=1, linestyles='solid', transform=ccrs.PlateCarree(),
            colors='g', zorder=3)
ax.clabel(dewp850, dewp850.levels, inline=True, fontsize=8)



# Define a skip to reduce the barb point density
skip_850 = (slice(None, None, 12), slice(None, None, 12))

# 850-hPa wind barbs
jet850 = ax.barbs(lon[skip_850], lat[skip_850], u_850[skip_850].m, v_850[skip_850].m, length=5,
                  transform=ccrs.PlateCarree(),
                  color='k', alpha=0.6, zorder=9)


mask_850 = ma.masked_less_equal(wspd_850, 30.0).mask
u_850[mask_850] = np.nan
v_850[mask_850] = np.nan
jet850_50 = ax.barbs(lon[skip_850], lat[skip_850], u_850[skip_850].m, v_850[skip_850].m, length=5,
                  transform=ccrs.PlateCarree(),
                  color='k', alpha=0.8, zorder=9)

mask_850 = ma.masked_less_equal(wspd_850, 50.0).mask
u_850[mask_850] = np.nan
v_850[mask_850] = np.nan
jet850_50 = ax.barbs(lon[skip_850], lat[skip_850], u_850[skip_850].m, v_850[skip_850].m, length=5,
                  transform=ccrs.PlateCarree(),
...
mask_850 = ma.masked_less_equal(wspd_850, 75.0).mask
u_850[mask_850] = np.nan
v_850[mask_850] = np.nan
jet850_50 = ax.barbs(lon[skip_850], lat[skip_850], u_850[skip_850].m, v_850[skip_850].m, length=5,
                  transform=ccrs.PlateCarree(),
...
#plt.savefig("analysis_850mb.png")
plt.show()